...
import { calculateGrade, createFallbackTeacherCommentary, TEACHER_COMMENTARY_VERSION } from "./gradingEngine.js";
  ...
  try {
    const response = await callStructured<unknown>(config, {
      ...
      prompt: "请为这份高中物理答卷生成简洁、客观、可供教师直接查看的评语。" +
        "\n\n硬性要求：" +
        "\n1. overallComment总结整体表现和实际得分，不得重新计算分数。" +
        ...
        "\n\n结构化评分结果：\n" +
        JSON.stringify({ score: result.score, maxScore: result.maxScore, reviewReasons: result.reviewReasons, subquestions: result.subquestions, evidenceLines: result.evidence.lines }, null, 2),
...
export async function gradeStudentAnswer(config: ModelConfigInput, input: {
  ...
  try {
    ...
    const evidence: AnswerEvidence = evidenceSchema.parse(extraction.data);
...
不得因为中间过程缺失或模糊而否定正确的最终答案。不得根据你自己求解题目得到的结果修改学生答案或参考答案。确定没有最终答案时返回missing；图像、转录或含义无法可靠确认时返回uncertain，不得把证据不足直接判为incorrect。每个小问必须恰好返回一条finalAnswerJudgement，并引用实际卷面行号。`,
...
${JSON.stringify(evidence)}`,
    ...
    try {
      const processResponse = await callStructured<unknown>(config, {
        model: config.visionModel,
        system: "你是受约束的评分点过程审验器。严格依据给定评分标准和卷面证据判断每一个评分点，不得自行解题或创造规则。最终答案状态由上一步教师模型权威确定，你不得修改它。即使最终答案状态为correct，也必须逐点评审并返回审验结论。你不能决定最终得分。明确满足返回satisfied，明确未满足返回not_satisfied，因字迹、涂改或证据不足无法确认返回insufficient_evidence并要求复核。核心公式正确时应判为satisfied，即使后续计算错误；同一错误不得重复触发扣分。",
...
${JSON.stringify(evidence)}`,
    ...
    completeOperation(operationId, "grading", "grading_completed", `${input.studentId}批改完成：${result.score}/${result.maxScore}分`, {
